 ...
 */
const PLACEHOLDERS = new Set(['change-me-access', 'change-me-refresh', 'change-me', 'changeme']);
...
export function requireSecret(config: ConfigService, key: string): string {
  ...
  if (!value) {
    throw new Error(`Falta ${key}. Genera los secretos con "pnpm setup" antes de arrancar la API.`);
  }
