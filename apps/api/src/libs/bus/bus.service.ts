...
export interface BusMessage<T = Record<string, unknown>> {
   ...
   * mensaje, y los consumidores descartan en silencio los que no lo traen. Era
   * opcional, y un punto de publicación llegó a mandar la cadena vacía — el
   * aviso se perdía sin que nada fallara ni lo registrara. Que el compilador lo
   * exija cuesta menos que descubrirlo en producción.
   */
  userId: string;
  botId?: string;
  type: string;
...
export class BusService implements OnModuleInit, OnModuleDestroy {
   ...
   * depósito de fichas de forma ATÓMICA. La API lo necesita por el mismo motivo
   * que el worker: sale por la MISMA IP, y los venues cuentan por IP. Sin esto,
