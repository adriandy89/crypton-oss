...
| `apps/api` → `modules/advisor` | El asesor: propone tres configuraciones al **crear** un bot. El modelo emite **perillas**, nunca parámetros. `openrouter.client.ts` es el único fichero que habla con un LLM, y sirve también al supervisor, al canal con IA y a los agentes, cada uno con su interruptor y su clave por llamada. |
...
| `apps/api` → `modules/ai-desk` | Los **agentes de IA** (spec 074): cada uno mira pares de una cuenta, y en cada vela el motor calcula las operaciones posibles y la IA —o el juez de reglas— **elige una letra** o ninguna. La propuesta se **recalcula** al aprobarla (botón de Telegram, app o automático: una sola rutina) y nace un bot `AGENT_TRADE` por `BotsService.create` + `START`. El seguimiento solo ciñe el stop o reduce la posición, por `updateConfig`. Todo se mide (`medicion.service.ts`), y lo medido vive aparte (`listados*`): nada de lo que decide lo importa. Rutas en `modules/admin/admin-ai-desk.controller.ts`; cada `ADMIN` ve solo sus agentes. |
...
| `apps/app` | Ionic 8 + Angular 21 + Capacitor. Ejecuta `strategy-core` **también en cliente** (`features/bots/bot-create.page.ts`, `fullConfig`). La pestaña **IA** (`features/ia`, specs 073-074: los agentes) es la única que no ve todo el mundo: solo un `ADMIN`, y lo que pide al servidor va bajo `RolesGuard` + `@Roles('ADMIN')`. |
...
| `packages/strategy-core` | Cada estrategia como funciones puras: `validate()`, `preview()`, `plan()`. Más `reconcile`, `order-gate`, `stop-loss`, `cycle-accounting`, `client-order-id`, `mutability`, el motor del canal con IA (`canal/`: régimen, canales, setups, herramienta, tasas base y juez), el de los agentes (`agentes/`: límites, familias, herramienta, juez, propuesta, seguimiento y medición) y la gestión de posición que comparten el canal y `AGENT_TRADE` (`operacion/gestion.ts`). `indicadores-vista.ts` envuelve esa misma estadística para lo que pinta el gráfico de la app (spec 061). |
...
14. **Ninguna IA escribe configuración ni fija un número por su cuenta.** El asesor, el supervisor, la IA del canal (`AI_CHANNEL`) y la de los agentes (`ai-desk`) emiten enumeraciones —nunca números, importes ni precios— y un generador determinista las traduce. La configuración pasa por `validate()`, `preview()`, `assertWithinLimits` y `diffConfig`, y solo cambia por `updateConfig`, como la de cualquier usuario: no hay un segundo camino de escritura de configuración. La IA del canal escribe una **intención de operación** (`bot_ai_intents`): elige entre las opciones que la herramienta ya calculó en el worker dentro de los topes del dueño, y el worker la vuelve a derivar con datos frescos, la revalida y la ejecuta por el mismo `plan()` → `reconcile()` → `place()`. El stop y los objetivos son órdenes nativas, ninguna salida espera al modelo y sin respuesta válida no hay entrada. Un agente escribe una **propuesta** con su plan en números, calculado por el motor; al aprobarla se **recalcula** con el precio de ahora y el bot nace por `BotsService.create` como cualquier otro. Su seguimiento solo ciñe el stop o baja el tope de posición, por `updateConfig`: lo que solo reduce el riesgo (`Strategy.soloReduceRiesgo`) no pasa los topes de creación, y ningún cambio que lo suba llega a aplicarse. Nada de lo que decide lee lo que se mide (R-25 del 074).
