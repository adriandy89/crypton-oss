...
| `packages/strategy-core` | Cada estrategia como funciones puras: `validate()`, `preview()`, `plan()`. Más `reconcile`, `order-gate`, `stop-loss`, `cycle-accounting`, `client-order-id`, `mutability`, el motor del canal con IA (`canal/`: régimen, canales, setups, herramienta, tasas base y juez) y el del «Bot de IA» (`trader/`: señal, la matriz de nueve, `cuantiza` y el juez de reglas, spec 068). `dimension.ts` es el dimensionado por tramos, que comparten los dos. `indicadores-vista.ts` envuelve esa misma estadística para lo que pinta el gráfico de la app (spec 061). |
