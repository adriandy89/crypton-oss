...
| 078 | [El modelo no llega a tiempo: agentes y canal](078-modelo-a-tiempo/spec.md) | cambio | hecho, antes del 076 (nace del primer agente real: con razonamiento medium el plazo de 20 s, topado en 25, cortaba cada ronda y se cobraba; plazo propio de 90 s, lo manual fuera de la peticion, el seguimiento cae al juez sin modelo —la decision 4 del 075—, la ronda fallida guarda sus candidatos y el coste desconocido se dice; faltan CA-8 y CA-9) | `spec/078-modelo-a-tiempo` |
| 079 | [Revision de parametros y calculos de todas las estrategias](079-revision-parametros-y-calculos/spec.md) | revision | hecho (28 hallazgos: 3 Altos, 17 Medios, 8 Bajos; el stop que salta detras de la liquidacion, los % de resultado sobre el precio y la Revision que no ensena ni stop ni resultado; la formula exacta de liquidacion coincide con la de Hyperliquid; todo lo corrige el 080 con las decisiones D-1 a D-6 del usuario. Durante el 080 salieron nueve mas, F-29 a F-37, anotados aqui y corregidos alli: el campo vacio que revienta, el tope que la Revision no aplicaba, la rejilla neutral con un solo presupuesto para los dos lados y la tendencia con el stop detras de la liquidacion) | `spec/079-revision-parametros-y-calculos` |
| 080 | [% sobre margen y una Revision de exchange profesional](080-roi-y-revision-profesional/spec.md) | cambio | hecho (corrige los 37 hallazgos del 079: stop y objetivos sobre el margen, una sola liquidacion exacta, stop que no puede quedar detras de la liquidacion, Revision por lados con resultado en USDC y ROI y el tope aplicado como en el motor, migracion de datos que aborta con bots en marcha, y todas las guias vueltas a calcular; falta CA-5, del usuario: crear el bot de la captura y ver la Revision nueva) | `spec/080-roi-y-revision-profesional` |
