...
| 069 | [TypeSafe decide: el «Bot de IA» ya tiene IA](069-typesafe-decide/spec.md) | cambio | hecho, sin desplegar (el modo IA se puede elegir y decide el modelo; arranca en sombra; falta CA-8, la comprobacion del usuario con un bot simulado) | `spec/069-typesafe-decide` |
| 070 | [Que el modelo evalue mas, y con mejor evidencia](070-cadencia-y-evidencia/spec.md) | cambio | hecho, sin desplegar (topes de llamada al dia, cadencia configurable y evidencia condicionada al estiramiento; MEDIDO con 996 llamadas reales: el modelo NO discrimina, y el bot no debe operar en modo IA con dinero) | `spec/070-cadencia-y-evidencia` |
| 071 | [¿Sabe el modelo distinguir una cascada de una repreciacion? Y la revision del Market Maker V2](071-cascada-medicion/spec.md) | medicion + correccion | hecho, sin desplegar (la medicion sale NEGATIVA: el edge de las cascadas era un artefacto de la ventana y no se construye el 072; la revision del MM V2 cierra 5 hallazgos, 2 Criticos, y sin bots corriendo sus valores medidos pasan a ser los de fabrica) | `spec/071-cascada-medicion` |
