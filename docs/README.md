...
| Canal (`AI_CHANNEL`) · solo administradores | **Alto** | Rebotes en el borde de un rango o canal, con el apalancamiento que permite el stop. El motor calcula las operaciones y una IA elige entre ellas. | [ai-channel.md](./ai-channel.md) | [`ai-channel.guide.ts`](../apps/app/src/app/core/content/ai-channel.guide.ts) | [`ai-channel.ts`](../packages/strategy-core/src/strategies/ai-channel.ts) |
| Bot de IA (`AI_TRADER`) · solo administradores | **Alto** | Toques del borde de una banda, con el apalancamiento que permite el stop. El motor calcula nueve operaciones posibles y una IA elige entre ellas. Su ventaja no está demostrada. | [ai-trader.md](./ai-trader.md) | [`ai-trader.guide.ts`](../apps/app/src/app/core/content/ai-trader.guide.ts) | [`ai-trader.ts`](../packages/strategy-core/src/strategies/ai-trader.ts) |
...
| [Canal](./ai-channel.md) | El reparto entre el motor y la IA, la regla del apalancamiento por stop, los límites que se comprueban tres veces, los avisos con botón de pausa y cómo empezar |
| [Bot de IA](./ai-trader.md) | Qué mide, la matriz de nueve operaciones, qué ve y qué no ve el modelo, y —sobre todo— lo que NO sabemos: su ventaja no está demostrada y depende del venue |
